...
# face_detector
base_options = python.BaseOptions(model_asset_path=f"{dir}/models/detector.tflite")
...
def apply_attention_detection_model(img):
    ...
    for prediction in predictions:
        ...
        print(prediction.boxes.data,'attention detected',classes_detected, labels)
...
def visualize(
  ...
  for detection in detection_result.detections:
    ...
    result_string, attentive_count, drowsy_count = apply_attention_detection_model(box_region)
...
def apply_face_detection_inference(image):
  ...
  annotated_image, attentive_count_all, drowsy_count_all  = visualize(image_copy, detection_result)
...
def object_detection_module(in_path):
  ...
  output_path = os.path.join(os.getcwd(),'public','videos', output_file_name)
  ...
  while True:
      ...
      if flag:
          ...
          frame_out, a_count, d_count =  apply_face_detection_inference(frame_rgb)
          a_count_all += a_count
          d_count_all += d_count
...
def create_video():
    try:
        ...
        if video and allowed_file(video.filename):
            ...
            output_filename, attentive_count, drowsy_count = object_detection_module(filepath)
